...
export class TableViewComponent<T> implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.user =
            this.loginService.getActualUser() != null
                ? this.loginService.getActualUser()
                : null;
        if (this.user != null) {
            for (
                let i = 0;
                i < this.modelService.getButtonPermissions().length;
                i++
            ) {
                let action = this.modelService.getButtonPermissions()[i];
                // @ts-ignore
                if (
                    action.roles.indexOf(this.user.roles[0]) >= 0 &&
    ...
    public changeKey(key: ViewKey, e: Event) {
        ...
        if (checked) {
            let indexKey = this.modelKeysTransformed.findIndex(
                (keyToFind: ViewKey) =>
                    keyToFind.privateKeyName === key.privateKeyName &&
                    keyToFind.publicKeyName === key.publicKeyName,
            );
            if (indexKey < 0) {
                let normalIndexPosition = this.modelKeys.findIndex(
                    (keyToFind: ViewKey) =>
                        keyToFind.privateKeyName === key.privateKeyName &&
                        keyToFind.publicKeyName === key.publicKeyName,
                );
            ...
            this.modelKeysTransformed = this.modelKeysTransformed.filter(
                (actualKey) => actualKey.privateKeyName !== key.privateKeyName,
            );
        }
    }
    ...
    public addSortRequest(key: string) {
        ...
        if (number === 0) {
            this.sort.push({ property: key, isAscending: true });
        } else if (number === 1) {
            for (let sortRequest of this.sort) {
    ...
    public weHaveContent(): boolean {
        if (!this.modelsTransformed) return false;
