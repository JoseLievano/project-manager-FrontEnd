...
export class AddNewBsTypeComponent implements OnInit, OnDestroy {
    ...
    private initialTaskCatCheck() {
        let taskCatVerification = this.bsTaskCategoryService
            .taskCategoriesAreEmpty()
            .subscribe({
                next: (response) => {
    ...
    private getTaskCatList(searchVal: string) {
        ...
        this.taskCatsFilterReq.operations = [
            new OperationRequest(':', searchVal, 'name'),
        ];

        this.taskCatsPageRequest.filter = [this.taskCatsFilterReq];

        let getTask = this.bsTaskCategoryService
            .getPageListView<bsTaskCategory>(this.taskCatsPageRequest)
            .subscribe({
                next: (response) => {
                    const content = response.content;
                    if (content) this.bsTaskCats = content;
                },
                error: (err) => {
                    this.errorService.processError(err);
                },
    ...
    public addNewType() {
        if (this.isValidNewType()) {
            ...
            let addNewType = this.bsTypeService
                .createNew(this.bsType)
                .subscribe({
                    next: (response) => {
                        if (response.id) {
                            const emitActionModel: ActionModelEmit<bsType> =
                                new ActionModelEmit<bsType>(
                                    actionType.NEW,
                                    response,
                                );
                            this.bsTypeService.modelsChanged.emit(
                                emitActionModel,
                            );
            ...
            this.alertService.addNewAlert(
                new UiMessage(
                    "Can't add the new type " + this.bsType.name,
                    messageType.ERROR,
                ),
            );
        }
        ...
        if (Array.isArray(this.bsType.taskCategories)) {
            this.bsType.taskCategories.forEach((actualTaskCat) => {
                if (typeof actualTaskCat == 'number') {
                    ...
                    if (actualTaskCat.id) numTaskCats.push(actualTaskCat.id);
                }
            });
        }
    ...
    public isValidNewType(): boolean {
        ...
        return !!(
            this.bsType.name &&
            this.bsType.name != '' &&
            this.amountOfTaskCat() > 0
        );
    }

    public addTaskCatToTypeModel(taskCat: bsTaskCategory) {
        if (
            typeof this.bsType.taskCategories != 'number' &&
            this.bsType.taskCategories
        ) {
            // @ts-ignore
    ...
    public deleteTaskCatFromType(taskCat: bsTaskCategory) {
        if (Array.isArray(this.bsType.taskCategories)) {
            ...
            this.bsType.taskCategories.forEach((actualTask) => {
                if (typeof actualTask != 'number') taskCats.push(actualTask);
            });
            const toDelIndex = taskCats.findIndex(
                (actualTaskCat) => actualTaskCat.id === taskCat.id,
            );
